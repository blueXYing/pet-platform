...
// Node 690:6660 (服务-商家详情页), 402x1306. This slice implements the 团购套餐
// (service project list / service cards) region against the frozen
// GET /api/v1/c/stores/{storeId}/services contract via a clearly-marked contract mock;
// store header / reviews remain design samples until /c/stores freezes. Booking itself
// belongs to a later C-003/C-004 slice and stays an explicit no-op.
type Phase = 'loading' | 'ready' | 'load-error' | 'expired' | 'invalid'
...
export default function StoreServicesPage() {
  ...
  return <ConsumerPageLayout page='storeServices' unit={unit} navigation={{ idPrefix: 'svc', disabled: !ready, onSelect: key => { setNotice(navigationUnavailableMessage(key)) }, referencePlacement: undefined }} className='svc-page' style={style}>
    ...
    {!ready && <View className='svc-state' role='status'>
      <Text>{phase === 'loading' ? '正在加载门店服务…' : phase === 'expired' ? '登录已失效，请重新登录' : phase === 'invalid' ? '门店参数无效' : '加载失败，请重试'}</Text>
    ...
    {ready && <StoreServicesDesign listTop={listTop} reviewTop={reviewTop} onBack={() => Taro.navigateBack().catch(() => setNotice('返回失败'))}
