...
export function createPrivateUploadTransport(origin: string, send: MultipartSender, allowLocalHttp = false): PrivateUploadTransport {
  ...
  return input => send({ url: `${origin}/api/v1/c/private-assets`, filePath: input.filePath, name: 'file', formData: { purpose: input.purpose ?? 'MERCHANT_APPLICATION_MATERIAL' }, header: { Authorization: input.authorization, 'X-Request-Id': input.requestId }, timeout: 60000 })
}
