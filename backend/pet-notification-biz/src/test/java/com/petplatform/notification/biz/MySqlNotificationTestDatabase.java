...
/**
 * Test-only, real MySQL fixture over the authoritative Schema 06. Never adopts or drops a
 * pre-existing database.
 */
final class MySqlNotificationTestDatabase implements AutoCloseable {
  private final String name =
      "mer_notification_test_" + UUID.randomUUID().toString().replace("-", "");
  private final JdbcTemplate admin;
  private final DataSource dataSource;
  private final JdbcTemplate jdbc;
  private boolean created;

  MySqlNotificationTestDatabase() throws Exception {
    ...
    if (!url.matches("jdbc:mysql://(127\\.0\\.0\\.1|localhost):[0-9]+/")) {
      throw new IllegalArgumentException(
          "PLAT003_MYSQL_URL must name a local isolated server, with no database or parameters");
    }
    ...
    try {
      ...
      while (root != null
          && !Files.exists(root.resolve("docs/03-database/06-核心数据库Schema-v0.1.sql"))) {
        root = root.getParent();
      }
