...
/**
 * Security chain for /api/v1/c/**: anonymous auth routes plus the session-filtered business
 * routes when C auth is enabled; everything else stays denied so no unimplemented route can be
 * reached accidentally. The chain only exists when the C slice is enabled — disabled, the
 * /api/v1/c/** surface keeps falling through to the global catch-all deny (403) like before.
 */
...
public class CSessionSecurityConfiguration {
    ...
    SecurityFilterChain cSecurity(
        ...
        http.authorizeHttpRequests(a -> {
            if (p.isEnabled()) {
                a.requestMatchers(HttpMethod.POST,
                        ...
                        "/api/v1/c/account/phone-binding").permitAll();
                a.requestMatchers(HttpMethod.GET,
                        "/api/v1/c/auth/session",
                        "/api/v1/c/pets",
                        "/api/v1/c/pets/*",
                        "/api/v1/c/profile").permitAll();
        ...
        http.exceptionHandling(e -> e
                .accessDeniedHandler((req, res, error) -> reject(req, res))
