...
/**
 * Protects the session-bearing /api/v1/c/** endpoints: resolves the MINIAPP Bearer into a
 * {@code MiniSessionView} request attribute before the controller runs. Anonymous auth routes
 * (attempts, wechat-login, phone-binding) and the convergent logout are not matched here.
...
public final class CBearerSessionFilter extends OncePerRequestFilter {
    ...
    private static final Set<String> PROTECTED_EXACT = Set.of(
            "/api/v1/c/pets", "/api/v1/c/profile", "/api/v1/c/auth/session");
    ...
    private static String traceJson(HttpServletRequest req) {
        ...
        if (value == null || !value.matches("[A-Za-z0-9._:-]{1,64}")) value = java.util.UUID.randomUUID().toString();
