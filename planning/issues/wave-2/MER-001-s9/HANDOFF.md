> 最新状态（2026-09-20）：真机上传、保存草稿、重新读取已由用户确认通过。位置限制已按用户裁决取消，不需要地图Key；默认仅校验输入格式，开放城市目录仍为成都。完整隔离申请/审核/协议/站内通知验收通过，身份入口使用测试替身。详见 PHONE-ACCEPTANCE.md。下文为此前阶段记录，旧地图待配置及真机待验收描述已被本状态覆盖。当前本地服务采用PRESERVE恢复模式。
...
后续本机发生一次真实续租超时保护，恢复时确认旧JVM退出，保留原数据库/Redis，使用此前不存在的node20，未重置node19高水位。手动服务已增加显式adopt模式：要求匹配元数据的原fixture数据库、已退出的前进程及未使用的新节点；此模式停止时保留数据。当前实际服务为PRESERVE，草稿v3及2份材料保留，新的登录attempt接口201通过。见local-server-recovery.json。该恢复工具只在test scope，未修改生产超时和fail-closed规则。

真机进度：用户已正确打开工程验证页，并确认手机浏览器能访问LAN接口得到预期401；小程序登录曾返回未确认错误，服务恢复后待重新验证。不能将预览/浏览器连通当作手机登录上传通过。临时防火墙规则MER001-LocalAcceptance-18081只放行D:/Python/python.exe、192.168.1.44:18081及192.168.1.0/24；验收服务不再需要时删除该专用规则。
