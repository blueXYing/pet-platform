...
| G1-08 / W2-SCHW-008 | T1 swap 店 A→B，T2 swap 店 B→A；或同店不同时间 swap；注入数据库 deadlock/锁超时 | 门店 ID 升序取得 guard，不产生永久死锁；异常整笔回滚，同 requestId 同参有界重试，原预约仍在直到新预约与释放一起提交。 |
...
| G2-01 / W2-SCHW-007/008 | 新员工无 capability 明细/集合头；两个会话 GET 都得 `[]/version=0`，随后不同 requestId PUT expectedVersion=0 | 唯一头行，恰一成功进 version=1；另一 409 并重读，明细不混合。 |
| G2-02 / W2-SCHW-007 | PUT expectedVersion=1 写空集合，后另一个会话持 version=0 或 1 写 | 头不删除，版本单调至 2；旧版本冲突；GET 空集合返回 version=2。 |
...
| G3-01 / W2-SCHW-002/004 | 接送 PICKUP 09:00–10:00、RETURN 12:00–13:00，分别有不同 `windowId`；改 RETURN 旧窗；改 PICKUP 旧窗 | 各自只保护对应 claim；被占用旧窗 close/降容/移时段均 409；改 kind/服务目标不能使旧 claim 消失。相邻半开区间仍可用。 |
...
1. **契约审阅**：SCH-003、ORDER、MER、SCH-004 和平台幂等 Owner 共同确认每条写路径的事务参与、锁序、当前事实 API、接送 claim 起止来源、当前指派唯一权威与失败映射。任何 Owner 暂未能提供事实时，相关减员/接送 hold 继续失败关闭。
