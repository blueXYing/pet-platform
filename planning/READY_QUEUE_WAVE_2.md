...
更新：2026-09-23，多角色并行轮：PR66（漂移修复）、PR67（MER-001门店读侧）、PR68（ADM-001服务写入方）、PR69（NTF服务审核通知消费侧）已逐级合并核验后顺序合入develop `25bf601`；PR70/71/72在途待用户合并（语法修复/通知boot接线/M-002+C-003前端+真实API联调取证）。本表区分完整Issue与已交付切片，不将局部验收升级为DONE。证据与遗留裁决清单见[2026-09-23收官记录](progress/2026-09-23/ROUND-CLOSEOUT.md)。
...
| C-003 / C-End | PR72交付门店列表/商家详情/服务详情切片（设计源还原+契约Mock+共享层真实API联调取证，CI绿待合并）；服务卡片部分此前已随PR72分支先行 | 模拟器VIS/真机留用户人工验收；评分/月售/距离/收藏等无契约字段不实现；预约入口仅跳转（不做预约表单，C004范围外） | MINI004/VIS；不做C004支付 |
| M-002 / Merchant | 签约页/准入契约/工作台真实准入已合入（PR61～63）；PR72交付服务管理列表/新增编辑页（设计源10:5255/11:5768确认+V1裁剪+20条VIS-004差异登记+NAVIGATION-BASIS导航依据），CI绿待合并 | 订单/排期/核销等其余工作台业务页、子账号、真机与VIS | MINI002～004/VIS；W2-FE |
