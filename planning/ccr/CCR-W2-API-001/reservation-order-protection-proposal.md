# SCH-003 / ORDER 预约、人员与指派保护联合 CCR 候选 v0.1

状态：**PROPOSED_FOR_REVIEW / NOT_APPROVED / NOT_IMPLEMENTED**，2026-09-24。本文仅提出后续契约，不修改已批准的 [34 号 API](../../../docs/04-api/34-Schedule-Protection-Contract-v0.1.md)、[34 号存储](../../../docs/03-database/34-Schedule-Protection-Storage-v0.1.md)或现行 06/07/10/11/12 号文件。四项 SCHC-1～4 已在[回执](schedule-write-completion-decisions.md)批准，不在本轮重问。本文不含 DDL、迁移、Java 实现、生产开关或测试通过声明。
...
**需要产品语义确认的三项**：一笔接送订单的“本次服务指派的员工”是否同一人承担两个完整方向窗；到店预约跨相邻 GENERAL 窗是否允许作为一笔预约；改期后原指派不覆盖新时间时是否可自动撤派。对应 ROC-2、ROC-3 的单窗部分和 ROC-6，均只是推荐，不能视为既有产品规则。**其余为技术契约选择**：全店共享人员可行性、ORDER 完整性查询与保护状态、预生成订单 ID 的绑定时序、物理索引与事务测试；它们不能借产品问题无限悬置。
...
| ROC-2 | 一笔接送订单仍只有一个 ORDER 当前服务人员；该人员须分别覆盖其 PICKUP 与 RETURN 两条完整 claim，中间空档不要求持续排班；不能为两个方向暗建两名“最终指派”。两个 claim 可分别与其他订单发生冲突，按整段半开区间判定。 | 推荐产品语义，基于最终 PRD 的单数指派字段；需明确批准。 |
| ROC-3 | 推荐新增“一笔到店预约须由**一条** OPEN、同店同服务 GENERAL 窗完整包容 `[appointmentStart,appointmentEnd)`”的产品选择；GENERAL claim 保存原 `windowId` 和实际预约区间。现有请求可在 guard 内仅当唯一包容窗时关联；后续 C 可约结果与下单/改期增 `selectedGeneralWindowId`，服务端仍校验区间和原窗。跨窗或歧义的新预约失败关闭，存量跨窗不伪造单 claim，逐单迁移/处置。 | **新增待批准产品选择**为单窗/跨窗；ID 与唯一关联是技术方案。不改变既有到店服务时长决定预约区间的规则。 |
...
| ROC-6 | 已有最终指派的订单改期时保留该指派；若原员工无法覆盖新时间，则改期失败、原预约与指派保持不变，不自动撤派。 | **新增待批准产品选择**；现有“先锁新、失败留旧”和“商家指派”并未明确这一分支。 |
...
**例 B（接送成对）**：甲只覆盖 PICKUP 10:00–11:00，乙只覆盖 RETURN 12:00–13:00，没有任何一人覆盖两段。ROC-2 获批前接送 hold 仍按已批边界关闭；获批后按同人约束返回 409。若另有丙分别完整覆盖两段且没有其他冲突，则本例可通过。不能以两个方向各有一人而通过。

**例 C（非贪心）**：旧未指派预约 X 可由甲/乙，新增 Y 仅甲，两段相交。先把 X 贪心放甲会误拒 Y；完整搜索把 X 放乙、Y 放甲可放行。没有商家改派发生。

## 4. ROC-3：到店 GENERAL 身份和跨窗

ROC-3 是**新产品选择候选**，不是对既有 PRD 的解释性重述。兼容期的到店命令仍携 `appointmentStart/appointmentEnd`；既有到店服务时长决定预约占用区间、服务端校验时长与实际区间的规则不变。SCH 在 guard 内读 OPEN GENERAL，候选要求仅一条同店同服务窗**完整覆盖已有规则算出的实际预约区间**，并保存该窗 ID 与原时间快照。只按开始点命中、取第一条、将相邻两个窗拼成一个逻辑窗均不合格；窗口关联步骤不得擅自重算或改写预约结束时间。即使新 HTTP 带 `selectedGeneralWindowId`，它只是防歧义的选择证据，服务端仍复查归属、状态、窗口包含及容量，不信任客户端时间。
...
跨相邻 GENERAL 窗的新预约若本来可以按连续营业时间完成，本推荐会提示重选并暂时不能下单；这是**需要产品明确批准的限制**，不能写成“既有 PRD 已禁止跨窗”。若产品要允许跨窗，应另定义用户选择几个窗、每段配置容量扣减、GENERAL claim 多条基数与服务时长区间，才可启用。存量跨窗订单维持履约，迁移时保留全部原始占用证据；不能硬关联其中一个窗或把它当空位。若选窗 ID 后仍出现同店同服务同 kind 的历史重叠窗，先隔离并修复数据，不靠 ID 掩盖重叠违约。
...
改期使用同一已存在的 `orderId`，新 claim 先经 ROC-1 验证，再在同事务替换旧 claim/预约与 ORDER 排期快照和改期次数；失败原事实不动。既有规则只要求先锁新、失败留旧，并允许商家指派/改派；**原指派不覆盖新时间时的结果尚未封板**。ROC-6 推荐 409 `SCHEDULE_SWAP_FAILED` 并保留原预约与指派，不自动清空商家指派；这是需批准的产品选择，不能在批准前实现为既定行为。若产品期望改期自动撤派、由商家重新指派，须另批准对应订单展示与流程；不能由 SCH 自作主张。
...
## 7. ROC-6：改期后的当前指派（新增待批准产品选择）

推荐订单改期在新时间继续保留商家当前指派，并把该员工作为 ROC-1 固定输入；员工若不覆盖新 GENERAL 或两条接送完整 claim，交换返回 `SCHEDULE_SWAP_FAILED`，原预约、原 claim、当前指派、改期次数与 30 分钟确认周期均不变。不能先写新排期再要求商家补人，也不能因另有合格但未指派员工而暗中改派。这个分支会限制用户可选的改期时间，属于产品选择，**需明确批准后实施**；若改为自动撤派，需再定义订单人员字段、商家通知和重新指派时限。本候选不改变“一单最多一次、同商家同服务、预约开始前、先锁新失败留旧”的已批规则。

## 8. 分期交付与验收门禁

1. **联合合同批准**：由 SCH-003、ORDER、MER、TX/客户端 Owner 核对 ROC-1～6，特别批准或改写 ROC-2 的同人、ROC-3 的单窗/跨窗及 ROC-6 的改期指派语义。批准前本候选不能当已实现 API。
