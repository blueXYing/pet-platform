...
**需要产品语义确认的三项**：一笔接送订单的“本次服务指派的员工”是否同一人承担两个完整方向窗；到店预约跨相邻 GENERAL 窗是否允许作为一笔预约；改期后原指派不覆盖新时间时是否可自动撤派。下文给出保守推荐，明确其用户可见影响。**其余为技术契约选择**：全店共享人员可行性、ORDER 完整性查询与保护状态、预生成订单 ID 的绑定时序、物理索引与事务测试；它们不能借产品问题无限悬置。
...
| ROC-3 | 新到店预约以**一条** OPEN、同店同服务 GENERAL 窗完整包容 `[appointmentStart,appointmentEnd)`；GENERAL claim 保存原 `windowId` 和实际预约区间。现有请求可在 guard 内仅当唯一包容窗时关联；后续 C 可约结果与下单/改期增 `selectedGeneralWindowId`，服务端仍校验区间和原窗。跨窗或歧义的新预约失败关闭，存量跨窗不伪造单 claim，逐单迁移/处置。 | 单窗/跨窗是产品影响点；ID 与唯一关联是技术方案。 |
...
**例 B（接送成对）**：甲可覆盖 PICKUP 10:00–11:00、RETURN 12:00–13:00；乙只覆盖前段，丙只覆盖后段。没有一个人能覆盖两段，则在 ROC-2 获批前接送 hold 仍关闭；获批后按同人约束返回 409。不能以两个方向各有一人而通过。

**例 C（非贪心）**：旧未指派预约 X 可由甲/乙，新增 Y 仅甲，两段相交。先把 X 贪心放甲会误拒 Y；完整搜索把 X 放乙、Y 放甲可放行。没有商家改派发生。

## 4. ROC-3：到店 GENERAL 身份和跨窗

兼容期的到店命令仍携 `appointmentStart/appointmentEnd`；SCH 在 guard 内读 OPEN GENERAL，要求仅一条同店同服务窗**完整覆盖**该区间，并保存该窗 ID 与原时间快照。只按开始点命中、取第一条、将相邻两个窗拼成一个逻辑窗、或用服务时长造结束时间均不合格；现有服务 duration 与区间一致性仍按原合同校验。即使新 HTTP 带 `selectedGeneralWindowId`，它只是防歧义的选择证据，服务端仍复查归属、状态、窗口包含及容量，不信任客户端时间。
...
改期使用同一已存在的 `orderId`，新 claim 先经 ROC-1 验证，再在同事务替换旧 claim/预约与 ORDER 排期快照和改期次数；失败原事实不动。若当前最终指派人不覆盖新时间，本候选推荐**409 `SCHEDULE_SWAP_FAILED` 并保留原预约与指派**，不自动清空商家指派。若产品期望改期自动撤派、由商家重新指派，须另批准对应订单展示与流程；不能由 SCH 自作主张。
...
getByReservationIds(storeId, reservationIds)
  -> 每个输入 ID 恰好一项：UNBOUND_IN_CURRENT_TX / UNASSIGNED / ASSIGNED /
     ...
     refundOrderState?, orderVersion, assignmentVersion?, protectRequired
...
listCurrentByStore(storeId, affectedStaffIds?)
  -> complete=true, source=PRIMARY_CURRENT_READ, storeId, totalCurrentCount,
     items[{orderId,reservationId,storeId,serviceId,currentStaffId,
            orderStage,verificationStatus,refundOrderState?,
...
保护口径按**底层事实**，不用 DisplayOrderStatus 推测：

| ORDER/预约组合 | 当前指派 `protectRequired` 与动作 |
|---|---|
| `PENDING_PAYMENT`、TEMP_LOCKED、无最终指派 | false；预约本身仍入 ROC-1 内部匹配。此阶段不开放商家最终指派。 |
| `PENDING_CONFIRM` 或 `PENDING_SERVICE`、未核销、预约 TEMP_LOCKED/CONFIRMED | 有当前指派则 true 并固定该员工；无指派仍入内部匹配。售后、退款申请、退款单创建或退款处理中不得提前解除 claim。 |
...
| `PENDING_CONFIRM/PENDING_SERVICE` 却已 RELEASED/EXPIRED、当前指派找不到对应订单/预约、两份当前人员不一致、未知状态或无可证明 claim | `INCONSISTENT`，全量查询报 503；不得将其排除为“零在途”，也不得通过减员。 |
...
1. **联合合同批准**：由 SCH-003、ORDER、MER、TX/客户端 Owner 核对 ROC-1～5，特别批准或改写 ROC-2 的同人与 ROC-3 的跨窗、改期指派语义。批准前本候选不能当已实现 API。
