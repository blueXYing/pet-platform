...
状态：`PROPOSED_CCR / NOT_APPROVED / NOT_IMPLEMENTED`
...
### 1.1 必须由产品/Contract Owner 确认的冲突决策

这些不是单纯实现参数，批准前不能由开发自行选一边：
...
在上述四项未获批前，可先实现表结构草案、只读模型和验证器原型，但不得把任一选择描述为生产规则或用fixture解除门禁。
